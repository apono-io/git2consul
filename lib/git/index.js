...
var dogstatsd = require('../dogstatsd').client();
...
exports.timedCreateRepos = function(config, cb) {
  var start = process.hrtime()
  this.createRepos(config, cb)
  end = process.hrtime(start)
  delta = (end[0] * 1000) + (end[1] / 1000000)
  logger.debug('git.createRepos execution time (hr): %dms', delta)
  dogstatsd.timing('git2consul.timing', delta, ['git:createrepos'])
}
